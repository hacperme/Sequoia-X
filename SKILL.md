...
- wrapper `/opt/data/scripts/sequoia_report.sh`：sync timeout 1800 容错(exit=124 降级) → backtest 1y 刷新缓存(失败沿用旧) → report → 摘要注入 agent；`SEQUOIA_FORCE=1` 跳过交易日检测（跨日验证）；防呆"滤后>0但区间内全0"→60s重试
