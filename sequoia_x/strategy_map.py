...
# 状态 → 策略建议（基于 2y 分状态净收益研究；2026-09-04 signal_mask 修复后重跑修正）
# 结构: {regime: {"focus": [策略名...], "avoid": [策略名...], "note": str}}
# ⚠️ 修正记录：高窄旗形旧实现缺'高位抗跌'致回测信号多于实盘（4340→1139），
#   修复后 2y 全状态不再正期望（5日胜率 48.7%→全状态 41%），已从主攻降级。
REGIME_ADVICE: dict[str, dict] = {
    ...
    "down_low": {
        "focus": [],
        "avoid": ["海龟突破", "RPS 突破"],
        "note": "阴跌低波动：样本少（高窄修复后亦无可靠正期望），整体观望为主",
    },
    "up_low": {
    ...
    "up_high": {
        ...
        "note": "高波上涨（情绪顶）：追高类历史全灭，防御为主，空仓或轻仓等回调",
    },
}

# 研究表数值快照（供日报"为什么"说明；来源 regime 分状态 2y 回测，修复后口径）
_REGIME_EVIDENCE = {
    "down_high": {"上升跌停": "+9.58%", "涨停洗盘": "+6.38%", "海龟突破": "+0.37%"},
    "up_low": {"海龟突破": "+2.63%", "均线放量": "+2.49%", "RPS 突破": "+2.82%", "上升跌停": "-1.84%"},
    "up_high": {"上升跌停": "-4.30%", "涨停洗盘": "-3.50%", "海龟突破": "-1.85%"},
    "down_low": {},
}
